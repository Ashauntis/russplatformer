...
class Game():
    def __init__(self):
        ...
        # buttons
        self.restart_button = Button(self.screen, screen_width // 2 -50, screen_height // 2 + 100, self.restart_img)
        self.start_button = Button(self.screen, screen_width // 2 - 350, screen_height // 2, self.start_img)
        self.exit_button = Button(self.screen, screen_width // 2 + 150, screen_height // 2, self.exit_img)
    ...
    def update(self):
        ...
        if main_menu:
            ...
            # Player has died
            if self.game_over == -1:
                self.draw_text('Game Over!', font, white, screen_width // 2 - 275, screen_height // 2 - 100)
            ...
            if self.game_over == 1:
                if self.level >= max_level:
                    self.draw_text('You Win!', font, white, screen_width // 2 - 240, screen_height // 2 - 175)
                    self.draw_text(f'Score: {score}', font, white, screen_width // 2 - 215, screen_height // 2 - 50)
                    if self.restart_button.draw():
                        self.level = 0
                        self.score = 0
                        self.world = self.reset_level(self.level)
